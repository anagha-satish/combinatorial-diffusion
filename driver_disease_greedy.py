#!/usr/bin/env python3
"""
Driver for Disease Graph Greedy Testing

Greedy policy (Positive Neighbors):
  - At each step, choose up to `budget` nodes from the current frontier
    that have the most positive neighbors in the underlying graph.
...
def run_greedy_budget(env, init_states, horizon=None):
    """
    Greedy policy respecting env.budget.
    Selects frontier nodes with the most positive neighbors in the underlying graph.
    ...
    # Pre-calculate adjacency matrix for counting positive neighbors
    # env.G has nodes labeled "X0", "X1", ...
    A = nx.to_numpy_array(env.G, nodelist=[f"X{i}" for i in range(env.n)])

    with tqdm(
        total=E, desc="greedy_positive", file=sys.stdout, mininterval=1.0, ncols=100
    ) as pbar:
        for ep in range(E):
            status = env.reset() # This gives obs, which is [status, mask]
            ...
            if init_states[ep] is not None:
                ...
                status = env.observation() # Refresh status if manual override

            ep_cum_pos = float(np.sum(env.status == 1))
            ep_cum_tests = float(np.sum(env.status != -1))

            last_reward = 0.0

            for t in range(horizon):
                # Greedy action selection
                mask = env.allowed_mask()
                candidates = np.flatnonzero(mask)
                
                if len(candidates) > 0:
                    # Count positive neighbors for all nodes
                    # env.status == 1 means positive
                    is_positive = (env.status == 1).astype(float)
                    positive_counts = A @ is_positive
                    
                    # Get counts for candidates
                    cand_counts = positive_counts[candidates]
                    
                    # Sort by count descending. 
                    # Use argsort. [::-1] reverses it.
                    sorted_indices_local = np.argsort(cand_counts)[::-1]
                    sorted_candidates = candidates[sorted_indices_local]
                    
                    # Select top budget
                    k = min(env.budget, len(sorted_candidates))
                    selected_nodes = sorted_candidates[:k]
                    
                    action = np.zeros(env.n, dtype=int)
                    action[selected_nodes] = 1
                else:
                    # No moves possible (shouldn't happen if game not done, but safety)
                    action = np.zeros(env.n, dtype=int)
                    k = 0
                ...
                if done:
                    ...
                    if t + 1 < horizon:
                        ...
                        # Cumulative stats stay constant after done
...
def main():
    ...
    for _ in range(args.n_episodes_eval):
        env.reset()
        init_states.append(env.status.copy())

    # -------- Run greedy --------
    out = run_greedy_budget(env, init_states, horizon=horizon)

    # -------- Output directory --------
    ts = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    run_dir = os.path.join(
        RESULTS_ROOT,
        f"{args.prefix}{args.std_name}_GreedyPositive_T{args.cc_threshold}_B{args.budget}_seed{args.seed}_{ts}",
    )
        ...
        [
            {
                ...
                "algo": "greedy_positive_neighbors",
        ...
        [
            {
                ...
                "greedy_disc_mean": float(final_disc.mean()),
            }
        ]
    )
    ...
    ax_curve.plot(mean_x, mean_y, lw=2, linestyle="dotted", label="greedy_positive_neighbors")
    ...
    ax_curve.set_title("Greedy Positive Neighbors")
    ...
    plt.savefig(
        os.path.join(
            run_dir,
            f"greedy_{args.std_name}_T{args.cc_threshold}_B{args.budget}_seed{args.seed}.png",
        ),
        dpi=300,
    )
    plt.close("all")

    print(f"✓ Greedy Positive Neighbors (budget-respecting) run saved to {run_dir}")
