...
# ---- JAX 0.6+ compatibility shim (also works on 0.4.x) ----
try:
    # JAX >= 0.6.0
    from jax.extend import core as jxcore
except Exception:
    # JAX <= 0.4.x fallback
    import jax.core as jxcore
    ...
    for mod_name, mod in sys.modules.items():
        if pattern.match(mod_name):
            for name, obj in getattr(mod, "__dict__", {}).items():
                if isinstance(obj, jxcore.Primitive):
...
def make_persist(f: Callable):
    ...
    def make_persist_f(*args) -> "PersistFunction":
        # use tree_util API (jax.tree.flatten is deprecated)
        ...
        assert all(getattr(x, "named_shape", {}) == {} and getattr(x, "sharding", None) is None
                   for x in in_descr_flat)
        assert all(getattr(x, "named_shape", {}) == {} and getattr(x, "sharding", None) is None
                   for x in out_descr_flat)
        ...
        if sig is not None:
            ...
            while len(arg_names) < len(args) and len(params) > 0:
                ...
                if param.kind in (inspect.Parameter.POSITIONAL_ONLY, inspect.Parameter.POSITIONAL_OR_KEYWORD):
                    arg_names.append(name); params.pop(0)
...
class PersistFunction:
    def __init__(
        self,
        closed_jaxpr: "jxcore.ClosedJaxpr",
        ...
        jaxpr: "jxcore.Jaxpr" = closed_jaxpr.jaxpr
        assert not getattr(jaxpr, "effects", ()), "Effects not supported"
    ...
    def __call__(self, *args):
        ...
        out_flat = jxcore.eval_jaxpr(self.closed_jaxpr.jaxpr, self.closed_jaxpr.consts, *in_flat)
    ...
    def pretty_print(self, use_color: bool = True):
        ...
        class CustomPrettyPrinter(PrettyPrinter):
            ...
            def _repr(self, obj, *args, **kwargs):
                if isinstance(obj, jax.ShapeDtypeStruct):
                    return f"{obj.dtype}[{','.join(str(x) for x in obj.shape)}]"
                return super()._repr(obj, *args, **kwargs)
        ...
        return (
            f"Exported with Jax version {jax.__version__}\n\n"
            f"--------- In ---------\n{in_info}\n\n"
            f"--------- Out ---------\n{out_info}\n\n"
