...
class _RFMService:
    ...
    @torch.no_grad()
    def sample(self, obs_np: np.ndarray, K: int, steps: int = 30, *,
               kappa: float | None = None, J_noise: int = 1) -> np.ndarray:
    ...
    @torch.no_grad()
    def sample_target(self, obs_np: np.ndarray, K: int, steps: int = 30, *,
                      kappa: float | None = None, J_noise: int = 1) -> np.ndarray:
        ...
        if obs.shape[1] != self.obs_dim:
            obs = obs[:, :self.obs_dim]
        Z = self.model_target.sample(obs, K=K, steps=steps)  # [B,K,D]
        if (kappa is None) or (J_noise <= 0) or (float(kappa) <= 0):
            return Z.detach().cpu().numpy()
        B, K_, D = Z.shape
        mu = Z.reshape(B * K_, D)
