...
class DPMDConfig:
    ...
    num_particles: int = 8
    target_J: int = 4       # candidates per next state for target V
...
class DPMD:
    ...
    We build critics once with the final flattened obs_dim.
    """
    ...
    def _target_v(self, next_obs: Tensor) -> Tensor:
        # Coerce width to what critics were built for
        next_obs = _coerce_width(next_obs, self.obs_dim)
        B = next_obs.shape[0]

        # Sample J candidates from RFM and average min-Q
        cand = rfm_service.sample(next_obs.detach().cpu().numpy(), K=self.cfg.target_J)  # [B, J, D]
        cand_t = _to_tensor(cand, self.device)                                           # [B, J, D]
        J = cand_t.shape[1]

        rep_obs = next_obs.unsqueeze(1).expand(B, J, self.obs_dim).reshape(B * J, self.obs_dim)
        flat_cand = cand_t.reshape(B * J, self.act_dim)

        q1 = self.tq1(rep_obs, flat_cand).view(B, J)
        q2 = self.tq2(rep_obs, flat_cand).view(B, J)
        return torch.minimum(q1, q2).mean(dim=1)  # [B]
    ...
    def update(self, batch: Experience) -> Dict[str, float]:
        ...
        # actor step via RFM (weighted flow-matching)
        with torch.no_grad():
        ...
        return {
            ...
            "q_mean": float(q_min.mean().detach().cpu()),
            "q_std": float(q_min.std().detach().cpu()),
        }
    ...
    def sample_candidates(self, obs: np.ndarray, K: int) -> np.ndarray:
        ...
        return rfm_service.sample(obs_np=o, K=K, steps=30)[0]
